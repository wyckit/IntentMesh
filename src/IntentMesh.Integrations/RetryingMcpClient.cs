...
public sealed class RetryingMcpClient : IMcpClient
{
    ...
    public RetryingMcpClient(IMcpClient inner, int maxAttempts = 3, Func<int, Task>? delay = null)
    {
        _inner = inner;
    ...
    public string CallTool(string name, IReadOnlyDictionary<string, string> arguments) => Run(() => _inner.CallTool(name, arguments));
